...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(listUser.size()==0){
           //iniDataBase
            DummieData dummieData = new DummieData(this);
    ...
    protected void onResume() {
        super.onResume();
        int id = GlobalData.id_user;
        if(GlobalData.id_user!=0){
            //When there is a user logged, he will be redirected to the Main Ativity
            GlobalData.appEnd = false; //evitamos que reinicie las variables cuando hace el intent
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                ...
                sqlModel = new SQLModel(this.context);
                sqlModel.open();
                ArrayList<Object[]> listUser = sqlModel.getUser(mEmail);
                if(listUser.size()> 0){
                    if(mPassword.equals(listUser.get(0)[2].toString())){
                        GlobalData.privileges = Integer.parseInt(listUser.get(0)[3].toString());
                        GlobalData.id_user = Integer.parseInt(listUser.get(0)[0].toString());
                        return true;
                    }else{
                        return false;
                    }
